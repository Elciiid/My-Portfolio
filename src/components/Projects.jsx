...
const ProjectCard = ({ project, index }) => (
  ...
  >
    ...
    <div className="relative overflow-hidden rounded-2xl aspect-video md:aspect-[4/3] w-full">
      <div className="absolute inset-0 bg-[#F48FB1]/20 mix-blend-overlay z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      <img 
        src={project.image} 
        alt={project.title} 
        className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700"
        onError={(e) => { e.target.src = 'https://placehold.co/600x400/222/F48FB1?text=Project'; }}
      />
    </div>

    {/* Content Section */}
    <div className="text-left w-full">
      <div className="text-[#F48FB1] text-xs md:text-sm font-bold uppercase tracking-widest mb-2">
        {project.type}
      </div>
      ...
      <a 
        ...
        className="inline-flex items-center gap-2 px-8 py-3 bg-white/5 border border-white/10 text-white font-medium rounded-full backdrop-blur-sm transition-all duration-300 hover:bg-[#F48FB1] hover:border-[#F48FB1] hover:shadow-[0_0_30px_rgba(244,143,177,0.4)] hover:-translate-y-1"
      >
